...
describe('<ReportPage />', () => {
  ...
  describe('getting real-time automation data', () => {
    it('should call updateReportData() successfully', async () => {
      const data = [{
        id: 1,
      }];
      const component = getComponent();
      const spy = jest.spyOn(component.instance(), 'updateReportData');

      component.instance().updateReportData(data);
      expect(spy).toHaveBeenCalled();
      expect(component.instance().state.reportData).toEqual(data);
    });
  });
});
